...
module.exports = {
    getDateFromString: (filePath) => {
        if (process.platform === "linux") {
            ...
            console.error(
                `getFileDate() => Error: only 'linux' and 'darwin' platforms are supported`
            );
            return null;
        }

        var getDateResult = execSync(cmd).toString();
        ...
        if (process.platform === "linux") {
            ...
            let creationDate = getDateResult
                .toString()
                .substring(start, getDateResult.length);
